...
var tagTopTracks = async function(genero, orden, direc){
  try{
    ...
    if(numResults==0){
      ...
      /* Obtención del JSON */
      console.log(`http://ws.audioscrobbler.com/2.0/?method=tag.gettoptracks&tag=${genero}&api_key=5a29d744e8273ab4a877e9b59555b81e&format=json&limit=50`)
      ...
      for(let i=0; i<response.length; i++){
        try{
          ...
          console.log(`http://ws.audioscrobbler.com/2.0/?method=track.getinfo&track=${response[i].name}&artist=${response[i].artist.name}&api_key=5a29d744e8273ab4a877e9b59555b81e&format=json&autocorrect=1&limit=1`)
          response[i] = await fetch(`http://ws.audioscrobbler.com/2.0/?method=track.getinfo&track=${response[i].name}&artist=${response[i].artist.name}&api_key=5a29d744e8273ab4a877e9b59555b81e&format=json&autocorrect=1&limit=1`);
          response[i] = await response[i].json();
          response[i] = response[i].track;
          /* Verificar undefined */
          if(typeof(response[i])==='undefined' || response[i].artist==undefined || response[i].album==undefined){
            console.log("ALBUM ELIMINADO");
            response.splice(i,1);
            i--;
            continue;
