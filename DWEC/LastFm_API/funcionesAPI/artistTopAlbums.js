...
var artistTopAlbums = async function(artista, orden, direc){
  try{
    ...
    if(numResults==0){
      ...
      /* Obtención del JSON */
      console.log(`http://ws.audioscrobbler.com/2.0/?method=artist.gettopalbums&artist=${artista}&api_key=5a29d744e8273ab4a877e9b59555b81e&format=json&autocorrect=1&limit=50`)
      response = await fetch(`http://ws.audioscrobbler.com/2.0/?method=artist.gettopalbums&artist=${artista}&api_key=5a29d744e8273ab4a877e9b59555b81e&format=json&autocorrect=1&limit=50`);
      response = await response.json();
      response = response.topalbums.album;
      console.log(response)
      /* Obtención del JSON Completo */
      for(let i=0; i<response.length && i<30; i++){
        try{
          /* Verificar undefined */
          ...
          response[i] = response[i].album;   // || response[i].mbid==''
          /* Verificar undefined */
