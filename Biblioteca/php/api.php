    ...
    try {
      ...
      if(isset($_GET['select'])){
        ...
        if($select=='libros'){
          ...
          //Se establece la consulta de reservas y préstamos activos
          $consulta2='SELECT COD_LIBRO FROM RESERVAS WHERE FECHA_FIN > CURDATE() UNION SELECT COD_LIBRO FROM PRESTAMOS WHERE DEVUELTO LIKE "No"';
          ...
          //Se realiza la consulta de libros
          ...
          //Se realiza la consulta de reservas y préstamos
          if (!$resp = mysqli_query($c1, $consulta2)){
            echo mysqli_error($c1).'<br>';
            echo 'Consulta: '.$consulta;
            exit(-1);
          }
          //Se almacenan los datos
          $data2 = mysqli_fetch_all($resp, MYSQLI_NUM);
          //Se transforma a una lista de ISBNs
          foreach($data2 as $i => $d) $data2[$i] = $d[0];
          //Se añade el campo DISPONIBLE a Libros
          foreach($data as $i => $d){
            if(in_array($data[$i]['COD_LIBRO'],$data2)) $data[$i]['DISPONIBLE']='No';
            else $data[$i]['DISPONIBLE']='Si';
          }
        }
        ...
        //MATRÍCULAS
        else if($select=='matriculas'){
          //Se forma la raíz de la consulta
          $consulta = 'SELECT ESTUDIOS, GRUPO, M.ALUMNO AS ALUMNO, NOMBRE, APELLIDOS, DNI, NIE FROM MATRICULAS M, ALUMNOS A WHERE M.ALUMNO=A.ALUMNO';
          //Se completa la consulta con los filtros corregidos
          foreach($filters as $i => $filt){
        ...
        //MATERIAS
        else if($select=='materias'){
          //Se forma la raíz de la consulta
          $consulta = 'SELECT DISTINCT MATERIA FROM LIBROS';
          //Se establece conexión con la BD
          $c1 = mysqli_connect($dbhost,$dbuser,$dbpass,$dbname) or die ('Error de conexion a mysql: ' . mysqli_error($c1).'<br>');
          //Se realiza la consulta
          if (!$resp = mysqli_query($c1, $consulta)){
        ...
        //RESERVAS
          ...
          //Se añade PENDIENTE a Préstamos
          foreach($data as $i => $d){
            if(strtotime($data[$i]['FECHA_FIN'])<time()) $data[$i]['PENDIENTE']='Si';
            else $data[$i]['PENDIENTE']='No';
          }
        ...
        //PRESTAMOS
          ...
          //Se añade PENDIENTE a Préstamos
          foreach($data as $i => $d){
            if(strtotime($data[$i]['FECHA_DEVOL'])<time()) $data[$i]['PENDIENTE']='Si';
            else $data[$i]['PENDIENTE']='No';
          }
          //Se corrigen las fechas
          foreach($data as $i => $dat){
          ...
          //Se establece la consulta de préstamos activos
          $consulta2='SELECT COD_LIBRO FROM RESERVAS WHERE FECHA_FIN > CURDATE() UNION SELECT COD_LIBRO FROM PRESTAMOS WHERE DEVUELTO LIKE "No"';
          //Se realiza la consulta
          if (!$resp = mysqli_query($c1, $consulta2)){
            echo mysqli_error($c1).'<br>';
          ...
          //Se transforma a una lista de ISBNs
          foreach($data2 as $i => $d) $data2[$i] = $d[0];
          //Se añade el campo DISPONIBLE a Préstamos
          foreach($data as $i => $d){
            if(in_array($data[$i]['COD_LIBRO'],$data2)) $data[$i]['DISPONIBLE']='No';
            else $data[$i]['DISPONIBLE']='Si';
          }
        ...
        //LIBROS_ACT_USUARIO
          ...
          $consultaR = 'SELECT COD_LIBRO, FECHA_FIN FROM RESERVAS WHERE FECHA_FIN > CURDATE()';
          //Se completa la consulta con los filtros corregidos
          foreach($filters as $i => $filt){
            $consultaR .= ' AND ';
          ...
          $consultaP = 'SELECT COD_LIBRO, FECHA_DEVOL FROM PRESTAMOS WHERE DEVUELTO="No"';
          //Se completa la consulta con los filtros corregidos
          foreach($filters as $i => $filt){
            $consultaP .= ' AND ';
          ...
          if (!$resp = mysqli_query($c1, $consultaR)){
            ...
            exit(-1);
          } else $data['reservas'] = mysqli_fetch_all($resp, MYSQLI_ASSOC);
          //Se realiza la consulta de Prestamos
          if (!$resp = mysqli_query($c1, $consultaP)){
            echo mysqli_error($c1).'<br>';
            echo 'Consulta: '.$consultaP;
            exit(-1);
          } else $data['prestamos'] = mysqli_fetch_all($resp, MYSQLI_ASSOC);
          //Se añade PENDIENTE a Reservas y se transforman sus fechas
          foreach($data['reservas'] as $i => $d){
            if(strtotime($d['FECHA_FIN'])<time()) $data['reservas'][$i]['PENDIENTE']='Si';
            else $data['reservas'][$i]['PENDIENTE']='No';
            $data['reservas'][$i]['FECHA_FIN']=date('d/m/Y',strtotime($d['FECHA_FIN']));
          }
          //Se añade PENDIENTE a Préstamos y se transforman sus fechas
          foreach($data['prestamos'] as $i => $d){
            if(strtotime($d['FECHA_DEVOL'])<time()) $data['prestamos'][$i]['PENDIENTE']='Si';
            else $data['prestamos'][$i]['PENDIENTE']='No';
            $data['prestamos'][$i]['FECHA_DEVOL']=date('d/m/Y',strtotime($d['FECHA_DEVOL']));
          }
        }
        ...
        //ADMINS
          ...
          $consulta = 'SELECT P.NOMBRE, P.APELLIDOS, D.DNI_JFK AS DNI, D.COD_DPTO, D.NOMBRE AS DEPARTAMENTO, D.CENTRO, PASSWORD FROM DEPARTAMENTOS D, PROFESORES P WHERE DNI_JFK=DNI AND DNI_JFK="'.$values[0].'"';
          //Se establece conexión con la BD
          $c1 = mysqli_connect($dbhost,$dbuser,$dbpass,$dbname) or die ('Error de conexion a mysql: ' . mysqli_error($c1).'<br>');
          //Se realiza la consulta
          if (!$resp = mysqli_query($c1, $consulta)){
        ...
        //RESERVAS CADUCADAS
        if($delete == 'reservasCad'){
          //Se forma la raíz de la consulta
          $query = 'DELETE FROM RESERVAS WHERE CURDATE() > DATE_ADD(FECHA_FIN, INTERVAL 1 WEEK)';
          //Se establece conexión con la BD
          $c1 = mysqli_connect($dbhost,$dbuser,$dbpass,$dbname) or die ('Error de conexion a mysql: ' . mysqli_error($c1).'<br>');
          //Se realiza la consulta
          if (!$resp = mysqli_query($c1, $query)){
